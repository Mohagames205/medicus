...
class VerificationLogger:
    ...
    async def send_embed(self, title: str, message: str, fields=None, color: discord.Color = discord.Color.blue(), external_message: str = ""):
        if fields is None:
            fields = []
    ...
    async def on_verified_user_join(self, member: discord.Member):
        await self.broadcast_info(title="Geverifieerde student gejoined", msg=f"{member.mention} was al eerder geverifieerd en heeft automatisch toegang gekregen.")
    ...
    async def broadcast_warning(self, msg: str, fields=None, title: str = "❗ Waarschuwing"):
        ...
        logging.info(msg + str_fields)
        await self.send_embed(external_message="<@&1157397256165658704>", message=msg, fields=fields, title=title, color=discord.Color.orange())

    async def already_verified(self, member: discord.Member):
        await self.broadcast_warning(title="Dubbele verificatie", msg=f"{member.mention} is al geverifieerd, maar probeert zich nogmaals te verifiëren. Mogelijks is hier iets misgelopen?")